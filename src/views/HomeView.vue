...
<template>
  <div
    class="home-view relative w-full min-h-dvh overflow-hidden"
    :style="homeStyle"
  >
    ...
    <div class="relative z-10 flex min-h-dvh flex-col px-0">
        ...
        >
          <div class="flex flex-col items-center justify-center gap-7 sm:gap-8">
            <img
              :src="Logo"
              alt="Logo Siculingo"
              class="h-24 w-auto drop-shadow-[0_8px_12px_rgba(0,0,0,0.22)] sm:h-32 md:h-36"
            />
            <div class="pointer-events-auto">
              <button
                type="button"
                class="inline-block rounded-full bg-[#E5C158] px-6 py-2.5 font-semibold text-[#111625] shadow-md transition hover:brightness-95"
      ...
      <div
        v-else
        class="quiz-page relative min-h-dvh w-full overflow-x-hidden overflow-y-auto bg-[#0A1230]"
        :style="quizPageStyle"
      >
        <div
          class="relative z-10 mx-auto flex min-h-dvh w-full max-w-7xl flex-col px-4 pb-6 pt-4 sm:px-8 sm:pt-6"
        >
          <div
            class="flex flex-1 items-center justify-center py-4 sm:-translate-y-[3rem] sm:py-6"
          >
            <Quiz />
          </div>
        </div>
